...
Plugin URI: http://plugins.191998.net/wp2tianya
Description: 同步发表 WordPress 博客日志到 天涯博客,初次安装必须设置后才能使用。更新时间2013.10.1
Version: 2.0.0
Author: yekong
Author URI: http://plugins.191998.net/wp2tianya
*/
class tianya{
...
function publish_post_2_tianya($post_ID){
	...
	if($post->post_date==$post->post_modified)
	{
		if($post->post_type =="post")
		{
				...
				elseif($sendurl==2)
				{
					$content=$content."<br/>查看原文：<a href=".get_permalink($post_ID).">".get_permalink($post_ID)."</a>";
				}
				elseif($sendurl==0)
				{
					$content=$content;
